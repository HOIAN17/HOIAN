...
class DDPG:
    ...
    #정의에서 env를 지웠다.
    def __init__(self, action, is_batch_norm=False): 
        #env.observation_space.shape[0] 를 지우고 스테이터스 개수를 넣는다.
        self.num_states = action.num_states
        #env.action_space.shape[0] 를 지우고 액션 개수를 넣는다.
        ...
        #각 액션 값들의 최댓값 배열을 대입해야 한다.
        action_max = np.array(action.high).tolist()
        #각 액션 값들의 최솟값 배열을 대입해야 한다.
        action_min = np.array(action.low).tolist()        
